...
export function normalizeTeamComponents(matrix){
 ...
 const populations=Object.fromEntries(COMPONENT_KEYS.map(k=>[k,(matrix.teams||[]).filter(t=>finite(t[k])).map(t=>Number(t[k]))]));
 const teams=(matrix.teams||[]).map(t=>({rosterId:t.rosterId,team:t.team,manager:t.manager,components:Object.fromEntries(COMPONENT_KEYS.map(k=>{const raw=finite(t[k])?Number(t[k]):null,pop=populations[k];return[k,{raw:raw!==null?raw:null,percentile:raw!==null&&pop.length?percentile(pop,raw):null,eligibleTeams:pop.length}];}))}));
