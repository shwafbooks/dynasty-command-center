...
export function buildTransactionFeed({transactions=[],rosters=[],users=[],players={},limit=100}){
  ...
  for(const tx of transactions){
    ...
    const headline=tx.type==='trade'?`${names.slice(0,2).join(' ↔ ')} trade`:tx.type==='waiver'?`${names[0]||'A team'} wins a waiver claim`:tx.type==='free_agent'?`${names[0]||'A team'} makes a roster move`:`${names[0]||'A team'} roster update`;
