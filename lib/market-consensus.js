...
export const MARKET_CONSENSUS_VERSION='market-consensus-v1';
...
export function buildMarketConsensus({sources=[],minimumSources=2}){
  ...
  const maps=usable.map(s=>({source:s.source,asOf:s.asOf||null,format:s.format||null,weight:Number.isFinite(Number(s.weight))?Number(s.weight):1,map:sourceMap(s)}));
  const ids=new Set(maps.flatMap(s=>[...s.map.keys()]));const players={};
  for(const playerId of ids){const evidence=[];for(const s of maps){const row=s.map.get(playerId);if(row)evidence.push({source:s.source,asOf:s.asOf,format:s.format,weight:s.weight,...row});}if(evidence.length<minimumSources)continue;const weightTotal=evidence.reduce((sum,e)=>sum+e.weight,0),consensus=evidence.reduce((sum,e)=>sum+e.percentile*e.weight,0)/weightTotal;const variance=evidence.reduce((sum,e)=>sum+e.weight*((e.percentile-consensus)**2),0)/weightTotal;players[playerId]={playerId,consensusPercentile:round(consensus),agreement:round(Math.max(0,1-Math.sqrt(variance))),sourceCount:evidence.length,evidence};}
  return{status:Object.keys(players).length?'consensus-ready':'insufficient-data',formulaVersion:MARKET_CONSENSUS_VERSION,minimumSources,sourceCount:maps.length,sources:maps.map(({map,...meta})=>meta),players,provenance:{formula:'within-source value rank -> percentile; weighted mean of source percentiles; agreement = 1 - weighted standard deviation of source percentiles',rawValuesAveraged:false,missingSourcePolicy:'A player must meet minimumSources. Missing providers are not treated as zero.',aiAdjusted:false,ageAdjusted:false,manualOverride:false}};
}

export function attachLeagueConsensus({teams=[],consensus}){const rows=[];for(const team of teams){const seen=new Set(),players=[];for(const p of [...(team.starters||[]),...(team.bench||[])]){const id=String(p.id||p.playerId||'');if(!id||seen.has(id)||!POSITIONS.includes(String(p.position||'').toUpperCase()))continue;seen.add(id);const c=consensus?.players?.[id];if(c)players.push({...c,name:p.name||p.fullName||id,position:String(p.position||'').toUpperCase()});}rows.push({rosterId:team.rosterId,team:team.team,manager:team.manager,matchedPlayers:players.length,players});}return{...consensus,teams:rows,provenance:{...consensus.provenance,leagueAggregation:'No team ranking is emitted by market-consensus-v1. Player consensus is evidence for a later audited roster formula.'}};}
