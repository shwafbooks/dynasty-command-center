...
import {canonicalFranchise,FRANCHISE_IDENTITY_VERSION} from './franchise-identity.js';
...
export async function buildLiveTeamComponentMatrix({leagueId,throughWeek}={}){
 ...
 const canonicalTeams=teams.map(t=>{const c=canonicalFranchise({leagueId,manager:t.manager,liveTeam:t.team,rosterId:t.rosterId});return{...t,team:c.team,manager:c.manager,liveTeam:c.liveTeam,canonicalTeam:c.canonicalTeam,canonicalManager:c.canonicalManager,identityStatus:c.identityStatus};});
 const production=buildLeagueTeamIntelligence({teams:canonicalTeams,seasonScoring});if(production.status==='insufficient-data')return{status:'withheld',reason:'verified-production-unavailable',verifiedWeeks:production.verifiedWeeks||[]};
